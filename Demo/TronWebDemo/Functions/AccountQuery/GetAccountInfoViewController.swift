...
class GetAccountInfoViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        self.title = "Account Info (Activation)"
        setupUI()
        updateNetworkStatus()
    }
    ...
    private func setupUI() {
        ...
        // Address Input
        addressLabel.text = "Enter TRON Address:"
        ...
        // Query Button
        queryButton.setTitle("Check Activation / Info", for: .normal)
    ...
    @objc private func handleQuery() {
        ...
        if address.isEmpty {
            let alert = UIAlertController(title: "Error", message: "Please enter a TRON address", preferredStyle: .alert)
        ...
        Task {
            if tronWeb.isGenerateTronWebInstanceSuccess != true {
                ...
                do {
                    ...
                    await executeGetAccount(address: address)
                } catch {
    ...
    private func executeGetAccount(address: String) async {
        queryButton.isEnabled = false
        resultTextView.text = "Fetching account info..."
        
        let response = await tronWeb.getAccountAsync(address: address)
