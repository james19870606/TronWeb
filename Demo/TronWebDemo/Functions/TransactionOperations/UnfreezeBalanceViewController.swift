...
class UnfreezeBalanceViewController: UIViewController {
    ...
    private let unfreezeButton = UIButton(type: .system)
    ...
    override func viewDidLoad() {
        ...
        self.title = "Unfreeze TRX (Stake 2.0)"
        setupUI()
        updateNetworkStatus()
    }
    ...
    private func setupUI() {
        ...
        // Amount
        amountLabel.text = "Amount to Unfreeze (TRX):"
        ...
        // Resource Type
        resourceTypeLabel.text = "Unfreeze Resource Type:"
        ...
        // Unfreeze Button
        unfreezeButton.setTitle("Unfreeze TRX", for: .normal)
        unfreezeButton.backgroundColor = .systemBlue
        unfreezeButton.setTitleColor(.white, for: .normal)
        unfreezeButton.layer.cornerRadius = 8
        unfreezeButton.addTarget(self, action: #selector(handleUnfreeze), for: .touchUpInside)
        contentView.addSubview(unfreezeButton)
        unfreezeButton.snp.makeConstraints { make in
        ...
        resultTextView.snp.makeConstraints { make in
            make.top.equalTo(unfreezeButton.snp.bottom).offset(20)
            make.left.right.equalToSuperview().inset(20)
    ...
    @objc private func handleUnfreeze() {
        ...
        Task {
            ...
            do {
                ...
                await executeUnfreeze(amount: amount, resourceType: resourceType, privateKey: privateKey)
            } catch {
                self.resultTextView.text = "TronWeb Setup Failed: \(error.localizedDescription)"
                self.unfreezeButton.isEnabled = true
            }
        }
    }
    
    private func executeUnfreeze(amount: Double, resourceType: String, privateKey: String) async {
        unfreezeButton.isEnabled = false
        resultTextView.text = "Unstaking TRX..."
        
        let response = await tronWeb.unfreezeBalanceAsync(amount: amount, resourceType: resourceType, privateKey: privateKey)
        self.unfreezeButton.isEnabled = true
