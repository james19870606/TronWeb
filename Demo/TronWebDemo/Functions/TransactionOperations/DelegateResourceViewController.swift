...
class DelegateResourceViewController: UIViewController {
    ...
    private let delegateButton = UIButton(type: .system)
    ...
    private func setupUI() {
        ...
        // Amount
        amountLabel.text = "Amount to Delegate (TRX):"
        ...
        // Resource Type
        resourceTypeLabel.text = "Delegate Resource Type:"
        ...
        // Receiver Address
        receiverLabel.text = "Receiver Address:"
        ...
        receiverTextField.placeholder = "T..."
        ...
        // Delegate Button
        delegateButton.setTitle("Delegate Resource", for: .normal)
        delegateButton.backgroundColor = .systemBlue
        delegateButton.setTitleColor(.white, for: .normal)
        delegateButton.layer.cornerRadius = 8
        delegateButton.addTarget(self, action: #selector(handleDelegate), for: .touchUpInside)
        ...
        resultTextView.snp.makeConstraints { make in
            make.top.equalTo(delegateButton.snp.bottom).offset(20)
            make.left.right.equalToSuperview().inset(20)
    ...
    @objc private func handleDelegate() {
        ...
        let receiverAddress = receiverTextField.text?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        ...
        if receiverAddress.isEmpty {
            showAlert(message: "Please enter receiver address")
            return
        }
        ...
        Task {
            delegateButton.isEnabled = false
            ...
            do {
                ...
                await executeDelegate(amount: amount, resourceType: resourceType, receiverAddress: receiverAddress, privateKey: privateKey)
            } catch {
                self.resultTextView.text = "TronWeb Setup Failed: \(error.localizedDescription)"
                self.delegateButton.isEnabled = true
            }
        }
    }
    
    private func executeDelegate(amount: Double, resourceType: String, receiverAddress: String, privateKey: String) async {
        delegateButton.isEnabled = false
        resultTextView.text = "Delegating resources..."
        
        let response = await tronWeb.delegateResourceAsync(amount: amount, resourceType: resourceType, receiverAddress: receiverAddress, privateKey: privateKey)
        self.delegateButton.isEnabled = true
