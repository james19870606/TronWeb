...
class TRC20TransferViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        self.title = "TRC20 Token Transfer"
        setupUI()
        updateNetworkStatus()
    }
    ...
    private func setupUI() {
        ...
        contentView.addSubview(contractTextField)
        contractTextField.snp.makeConstraints { make in
            make.top.equalTo(contractLabel.snp.bottom).offset(8)
            make.left.right.equalToSuperview().inset(20)
        ...
        fromAddressLabel.snp.makeConstraints { make in
            make.top.equalTo(contractTextField.snp.bottom).offset(15)
            make.left.equalToSuperview().offset(20)
        }
        ...
        // Transfer Button
        transferButton.setTitle("Send TRC20 Token", for: .normal)
    ...
    @objc private func handleEstimate() {
        ...
        if contract.isEmpty || fromAddress.isEmpty || toAddress.isEmpty {
            showAlert(message: "Please fill all fields (Contract, From, To) to estimate fee")
            return
        }
